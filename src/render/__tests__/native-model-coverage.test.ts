...
import { CONSTRUCT_ENEMY_IDS, RETAINED_LICENSED_RIG_IDS } from "../construct-model";
...
const RETAINED_KEYFRAMED_DRAGONS = [
  "enemy.dragon.fire",
  "enemy.dragon.ice",
  "enemy.dragon.hydra",
...
describe("complete live enemy model coverage", () => {
  it("accounts for all 61 live enemies with native or explicitly retained rigs", () => {
    const covered = [
      ...ARACHNID_ENEMY_IDS,
    ...
    for (const id of RETAINED_KEYFRAMED_DRAGONS) expect(ENEMIES[id].view).toBe("dragon");
    expect(ENEMIES[RETAINED_LICENSED_RIG_IDS[0]].view).toBe("warden");
  });
  ...
  it("keeps every native family on a dedicated animation branch and shared combat hooks", () => {
    ...
    for (const family of ["arachnid", "undead", "construct", "ooze", "canid", "ungulate", "raider", "flier", "signature"]) {
