...
- **Everything in the world is made of cubes (HARD RULE).** Every visible thing
  ...
  **No free-floating thin quads, wedges, cones, cylinders, spheres, or
  ...
  slab/stair/post, stop and rebuild it from cubes. Bridges: the deck is **full
  1×1×1 plank blocks** (one full block thick, so a height change reads as a clean
  block step) and the supports are **full 1×1 stone columns** (stacked cubes),
  with the river rendered **underneath** — never a filled span and never a void
  below. Audit new world geometry against this before shipping.
