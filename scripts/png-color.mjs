// Average color of a PNG, plus a vanilla-texture lookup for the structure
// baker. Only a computed hex color ever leaves this module — no third-party
// pixels are stored or shipped.

import { execFileSync } from "child_process";
import { inflateSync } from "zlib";

/**
 * Decode a PNG (bit depth 8 for gray/RGB/RGBA/gray+alpha; depths 1/2/4/8
 * for palette) into full RGBA pixels, or null when unsupported.
 */
export function decodePng(buffer) {
  try {
    ...
    const unfilter = (offset, rows, rowBytes) => {
...
/** Alpha-weighted average color of a PNG as "#rrggbb", or null. */
export function averagePngColor(buffer) {
  ...
  for (let i = 0; i < width * height * 4; i += 4) {
    const a = rgba[i + 3];
    r += rgba[i] * a; g += rgba[i + 1] * a; b += rgba[i + 2] * a; wsum += a;
  }
...
const TEXTURE_BASE = "https://raw.githubusercontent.com/misode/mcmeta/assets/assets/minecraft/textures/block";

/**
 * Look up a block's vanilla texture (mcmeta mirror of unmodified assets)
 * and return its average color. Tries the usual texture-name variants;
 * returns { color, texture } or null when offline / not found.
 */
export function lookupVanillaBlockColor(blockName) {
  const base = blockName.replace(/^waxed_/, "");
  ...
  for (const candidate of candidates) {
    try {
      const png = execFileSync("curl", ["-sfL", "--max-time", "10", `${TEXTURE_BASE}/${candidate}.png`], {
        maxBuffer: 8 * 1024 * 1024,
      });
      const color = averagePngColor(png);
      if (color) return { color, texture: `${candidate}.png` };
    } catch {
      // not found / offline — try the next candidate
    }
  }
  return null;
}
